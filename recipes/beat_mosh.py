...
from datamosh import (MoshConfig, chroma_databend, ffmpeg, make_moshable,
                      mosh_segment, parse_avi, paths, pixel_sort,
...
PIXELSORT = dict(mode="threshold", key="hue", direction="h",
                 frac=0.2, lo=64, hi=192, reverse=False)  # frac 0 disables
...
CFG = MoshConfig(keyframe_delete_prob=0.0,
                 escalate=1,
...
w, h = (WIDTH, HEIGHT) if WIDTH and HEIGHT else Counter(
    ffmpeg.dimensions(s) for s in sources).most_common(1)[0][0]
w, h = w - w % 2, h - h % 2  # post effects require even dimensions
fps = FPS or Counter(round(ffmpeg.frame_rate(s), 3)
                     for s in sources).most_common(1)[0][0]
    ...
    if not (os.path.exists(dst) and os.path.getmtime(dst) >= os.path.getmtime(src)):
        ...
        subprocess.run(
            ["ffmpeg", "-y", "-hide_banner", "-loglevel", "error", "-i", src,
             "-vf", vf, "-an", "-c:v", "libx264", "-crf", "16", "-preset", "fast",
             "-f", "avi", dst],
            check=True)
    clips.append(dst)
...
for src in clips:
    ...
    dst = os.path.join(mosh_dir,
                       f"{stem}_{BPM:g}bpm_{CUT_BEATS[0]:g}-{CUT_BEATS[1]:g}b.avi")
...
while cum * unit < target:
    ...
    intensity = 1 + CFG.escalate * min(1.0, cum * unit / target) if CFG.escalate else 1.0
    moshed, prev_pframes = mosh_segment(CFG, seg, keep_keyframe=(i == 0),
                                        donor_pframes=prev_pframes, intensity=intensity)
    ...
    out.extend(kept)
    last_v = kept[-1]
    print(f"[{i}] {name}: {len(seg)}f section -> {k / GRID_DIV:g}-beat cut "
          f"({quota}f){' (melt)' if melt else ''}")
    cum += k
    i += 1
grid = paths.resolve(GRID_AVI)
write_avi(grid, header, movi_start, out)
print(f"wrote {grid} ({len(out)} frames over {cum / GRID_DIV:g} beats, "
      f"{melts} melts)")
...
subprocess.run(
    ["ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
     "-i", cur, "-ss", f"{SONG_START:.3f}", "-i", song,
     "-map", "0:v:0", "-map", "1:a:0",
