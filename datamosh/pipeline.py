...
def run_mosh(cfg=None, *, shots=None, progress=None):
    ...
    for it in range(cfg.n):
        ...
        intensity = 1 + cfg.escalate * (it / max(1, cfg.n - 1)) if cfg.escalate else 1.0
        ...
        if progress:
            progress((it + 1) / cfg.n, f"clip {it + 1}/{cfg.n}")
