...
def add_config_args(ap):
    ...
    ap.add_argument("--source", default=None, help=f"source video (default {d.source})")
    ap.add_argument("--output", default=None, help=f"output AVI path (default {d.output})")
    ap.add_argument("--n", type=int, default=None, help=f"segments to append this run (default {d.n})")
    ap.add_argument("--seed", type=int, default=None, help="RNG seed (same seed = same result)")
    ap.add_argument("--min-shot", type=float, default=None,
    ...
    ap.add_argument("--no-fixup", action="store_true", help="skip the ffmpeg -c copy pass")

    tune = ap.add_argument_group("tunables (see `python -c \"import datamosh; datamosh.describe()\"`)")
    for f in float_fields():
        tune.add_argument(_flag(f.name), type=float, default=None, metavar="F",
                          help=f"{f.metadata['help']} (default {f.default})")
    for f in range_fields():
        lo, hi = f.default
        tune.add_argument(_flag(f.name), type=int, nargs=2, default=None,
                          metavar=("LO", "HI"),
...
def config_from_args(args):
    ...
    updates = {}
    for name in ("source", "output", "n", "seed", "min_shot", "scene_threshold"):
